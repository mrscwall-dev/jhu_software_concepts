...
DATA_FILE = Path(__file__).with_name("llm_extend_applicant_data.json")
...
def read_applicant_data():
    """Read the cleaned Module 2 applicant data."""
...
def prepare_record(applicant):
    """Map a cleaned Module 2 applicant record to the database columns."""

    return {
        "p_id": get_p_id(applicant.get("applicant_url")),
        "program": applicant.get("program"),
        "comments": applicant.get("comments"),
        "date_added": parse_date(applicant.get("date_added")),
        "url": applicant.get("applicant_url"),
        "status": applicant.get("status"),
        "term": applicant.get("program_start"),
        "us_or_international": applicant.get("applicant_type"),
        "gpa": applicant.get("gpa"),
        "gre": applicant.get("gre_score"),
        "gre_v": applicant.get("gre_verbal"),
        "gre_aw": applicant.get("gre_aw"),
...
def load_data():
    ...
    try:
        ...
        with psycopg.connect(DATABASE_URL) as connection:
            with connection.cursor() as cursor:
                ...
                for applicant in applicants:
                    record = prepare_record(applicant)

                    if record["p_id"] is None:
                        skipped += 1
                        continue
        ...
        print(f"Applicant records read: {len(applicants)}")
