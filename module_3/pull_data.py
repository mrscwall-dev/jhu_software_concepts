...
EXTENDED_DATA_FILE = BASE_DIR / "llm_extend_applicant_data.json"
...
def collect_new_records():
    ...
    while True:
        current_url, html = scraper.wait_for_results(timeout=120)
        records = scraper.scrape_data(html)

        found_existing = False

        for record in records:
            p_id = get_p_id(record.get("applicant_url"))
...
def standardize_new_records(new_records):
    ...
    os.environ.setdefault(
        "CANON_UNIS_PATH",
        str(BASE_DIR / "llm_hosting" / "canon_universities.txt"),
    )
    os.environ.setdefault(
        "CANON_PROGS_PATH",
        str(BASE_DIR / "llm_hosting" / "canon_programs.txt"),
    )
    ...
    for record in new_records:
        ...
        extended_record["program"] = make_program(record)
        ...
        extended_record["llm-generated-university"] = (
            result["standardized_university"]
        )

        standardized_records.append(extended_record)
...
def update_raw_json(new_records):
    ...
    added = merge_data(existing_data, new_records)
...
def insert_into_database(records):
    ...
    with SessionLocal() as session:
        for record in records:
            ...
            if session.get(Applicant, p_id) is not None:
                continue
...
def pull_data():
    ...
    if not new_records:
        print("No new GradCafe records were found.")
        return

    update_raw_json(new_records)

    standardized_records = standardize_new_records(new_records)

    update_extended_json(standardized_records)

    inserted = insert_into_database(standardized_records)

    print(f"New GradCafe records found: {len(new_records)}")
    print(f"New database records inserted: {inserted}")
